  ...
  const analyzePrescription = async () => {
    ...
    try {
      const res = await fetch('/api/advanced/ai/analyze-prescription', {
        ...
        body: JSON.stringify({ image, lang: i18n.language })
      });
      ...
      <header className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          <h1 className="text-4xl md:text-6xl font-display font-bold text-white tracking-tighter uppercase">
            {t('ai_prescription')} <span className="text-blue-500">{t('scanner')}</span>
          </h1>
          <p className="text-gray-500 font-medium mt-2">{t('prescription_desc')}</p>
        </div>
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-2 px-4 py-2 bg-blue-500/10 border border-blue-500/20 rounded-full">
            <ShieldCheck className="w-4 h-4 text-blue-400" />
            <span className="text-[10px] font-bold text-blue-400 uppercase tracking-widest">{t('hipaa_compliant')}</span>
          </div>
          <div className="flex items-center gap-2 px-4 py-2 bg-emerald-500/10 border border-emerald-500/20 rounded-full">
            <Zap className="w-4 h-4 text-emerald-400" />
            <span className="text-[10px] font-bold text-emerald-400 uppercase tracking-widest">{t('real_time_analysis')}</span>
          </div>
        </div>
      </header>
        ...
        <div className="space-y-6">
          <div className="bg-white/2 border border-white/5 rounded-[2.5rem] p-8 lg:p-12 relative overflow-hidden group">
            <div className="relative z-10">
              <h3 className="text-2xl font-display font-bold text-white mb-6">{t('upload_prescription')}</h3>
              ...
              <div className="aspect-[4/3] bg-black/40 rounded-[2rem] border-2 border-dashed border-white/10 flex flex-col items-center justify-center relative overflow-hidden group/upload">
                {image ? (
                  ...
                  <label className="w-full h-full flex flex-col items-center justify-center cursor-pointer hover:bg-white/5 transition-all">
                    ...
                    <p className="text-sm font-bold text-white uppercase tracking-widest">{t('drop_image_upload')}</p>
                    <p className="text-xs text-gray-500 mt-2">{t('supports_formats')}</p>
                    <input type="file" accept="image/*" className="hidden" onChange={handleImageUpload} />
                  </label>
                )}
              </div>
          ...
          <div className="bg-white/2 border border-white/5 rounded-[2.5rem] p-8">
            <h4 className="text-xs font-bold text-gray-500 uppercase tracking-[0.3em] mb-6">{t('safety_information')}</h4>
            ...
            {isAnalyzing ? (
              ...
              >
                ...
                <p className="text-gray-500 text-sm max-w-xs leading-relaxed">
                  {t('extracting_medical_data_desc')}
                </p>
              </motion.div>
            ) : result ? (
              <motion.div
              ...
              >
                ...
                <h3 className="text-2xl font-display font-bold text-white mb-4">{t('analysis_error')}</h3>
              ...
              >
                ...
                <h3 className="text-2xl font-display font-bold text-gray-700 mb-4">{t('ready_for_analysis')}</h3>
                <p className="text-gray-600 text-sm max-w-xs leading-relaxed">
                  {t('upload_prescription_placeholder')}
                </p>
              </motion.div>
            )}
          </AnimatePresence>
